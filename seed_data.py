...
def creer_produits():
    ...
    produits_data = [
        ...
        {"nom": "Sucre semoule 1kg", "prix": 12.00, "stock": 120, "seuil": 20, "categorie": "Épicerie"},
        {"nom": "Farine blanche 1kg", "prix": 8.50, "stock": 80, "seuil": 15, "categorie": "Épicerie"},
        ...
        {"nom": "Café Moulu 200g", "prix": 45.00, "stock": 18, "seuil": 5, "categorie": "Épicerie"},
        {"nom": "Thé vert 100g", "prix": 28.00, "stock": 42, "seuil": 12, "categorie": "Épicerie"},
...
def creer_clients():
    ...
    for client_data in clients_data:
        ...
        if created:
            ...
            print(f"  ⚠️  Client existe déjà: {client.nom}")
...
def creer_employes():
    ...
    for emp_data in employes_data:
        ...
        if created:
            print(f"  ✅ Employé créé: {employe.nom}")
        else:
            print(f"  ⚠️  Employé existe déjà: {employe.nom}")
...
def creer_ventes():
    ...
    if not produits:
        print("  ❌ Aucun produit trouvé. Créez d'abord les produits.")
        return
    ...
    for i in range(20):  # 20 ventes aléatoires
        ...
        try:
            ...
            ventes_creees += 1
            print(f"  ✅ Vente créée: {vente.numero_transaction} ({vente.produit.nom} x{vente.quantite})")
            
        except Exception as e:
            print(f"  ❌ Erreur lors de la création de la vente: {str(e)}")
    
    print(f"\n  📊 Total ventes créées: {ventes_creees}")
...
def creer_approvisionnements():
    ...
    fournisseurs = ["Fournisseur Alimentaire SA", "Boissons du Maroc", "Épicerie Centrale", "Laiterie Nord"]
    ...
    for i in range(8):  # 8 approvisionnements
        ...
        try:
            ...
            print(f"  ✅ Approvisionnement créé: {approvisionnement.produit.nom} (+{quantite})")
            
        except Exception as e:
            print(f"  ❌ Erreur lors de la création de l'approvisionnement: {str(e)}")
...
def creer_paies():
    ...
    for employe in employes:
        ...
        try:
            ...
            print(f"  ✅ Paie créée: {employe.nom} - {paie.salaire_calcule:.2f} MAD")
            
        except Exception as e:
            print(f"  ⚠️  Paie existe déjà ou erreur: {employe.nom} - {str(e)}")
